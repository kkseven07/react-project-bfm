...
export default ({page,book} ) =>(
    ...
    <div className="full " styleName="r">
        <div className="absolute" styleName={setStyle(page.primary_image.image.url)[0]}>
            <div styleName="day" style={{display:"none"}}>{getDate(book.dob).day}</div> 
            <div styleName="month" style={{display:"none"}}>{getDate(book.dob).monthNumber}</div>
            <div styleName="year">
              {setStyle(page.primary_image.image.url)[0].match('v3')?(""+getDate(book.dob).year).substr(2,3):getDate(book.dob).year}
            </div>               
        </div>
        <div className="absolute" styleName={setStyle(page.primary_image.image.url)[1]} style={{fontSize:checkLength(page.data.film, 18, 2)}}>
            {page.data.film}
        </div>
    </div>
);
