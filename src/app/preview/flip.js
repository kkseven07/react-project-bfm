...
class FlipPage extends Component {
  ...
  stopMoving (e) {
  var goNext, goPrevious
    if(this.state.angle > 0) this.moving = true
    const delay = Date.now() - this.state.timestamp
  if(this.state.angle !== 0){
      goNext = !this.isLastPage() && (
        this.state.angle <= -90 ||
    ...
    goNext = !this.isLastPage() && (
        this.state.angle === 0 && (e.pageX > 675 && !this.moving)
        )
    goPrevious = !this.isFirstPage() && (
        this.state.angle === 0 && (e.pageX < 675 && !this.moving)
        )
    ...
    this.setState({
      ...
      if(this.state.angle === 0)  this.moving = false
      // else this.state.angle = 0
    })
  }
