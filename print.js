...
let convertBooklet = (names, book_id) => {
    fs.readdir(`../print/${book_id + "-print"}/`, (err, files) => {
        let filenames = names
            .map(
                (type, i) =>
                    `../print/${book_id + "-print"}/${files.filter(
                        file => file.indexOf(type) > -1
                    )[0]}`
            )
            .join(" ");
        shell.exec(
            `convert ${filenames} ../print/${book_id +
                "-print"}/${book_id}booklet.pdf`
        );
        // shell.exec(
...
let printBooklet = (urls, book_id) => {
    shell.exec(
        `electroshot [${urls} 1896x976]  --delay 7500  --out ../print/${book_id +
...
let getPrintUrls = (book_type, book_id, types_arr) => {
    ...
    // .filter(concept_pair => {
    //     return (
    //         concept_pair.indexOf("relaxPhoto") > -1 ||
    //         concept_pair.indexOf("pastPhoto") > -1 ||
...
let print = (urls, book_id) => {
    ...
    //             // v.indexOf("animal") > -1 ||
    //             v.indexOf("qualityTableChoice") > -1||
    //             v.indexOf("endPage") > -1
    //     )
    //     .join(" ");
    shell.exec(
